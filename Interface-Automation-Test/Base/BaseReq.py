...
class Config(object):
    ...
    def form_Eng(self, zh_char):
        return zh_char.replace('\r', '').replace(' ', '').replace('\n', '').replace('\t', '').replace('：', ':').replace('，', ',').replace('“', '"').replace('”', '"').replace('【', '[').replace('】', ']')
        #replace('欲替换的符号', '新符号')
    ...
    def config_req(self, kw, token):
        ...
        header = {
            ...
            'Origin': 'http://192.168.16.60',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.97 Safari/537.36',
            'Referer': 'http://192.168.16.60/chat',
        ...
        for item in kw:
            if item["接口名"] == "$$$$$":#连续5个$，报告要分页，直接向数据文件中写入报告分页信息
                ...
                try:#断言json异常检测
                    ...
                    continue
                """
                # 只需看懂一段，其他都是不同的判断  一样的操作
                if item["请求方式"] == "POST" or "post":
                    try:
                        if item["json"] == "是":# 判断请求报文是否为json格式，如是，则传参方式为json传参，否则为data                #############
                            if item["保存id"] == "是":  # 判断请求是否保存id，如需要，则保存id，供后续接口使用该参数
                                res = requests.post(url=url, json=jsondata, headers=header, verify=False, timeout=10)                ###########
                                id = res.json()["demandId"]
                            elif item["沿用id"] == "是":  # elif逻辑处理需要用到上一个请求返回的参数id； 判断请求是否需要沿用id，如需要，则使用保存的id
                                try:
                                    if jsondata["demandId"]:
                                        jsondata["demandId"] = str(id)
                                    else:
                                        pass
                                except:
                                    if jsondata["id"]:
                                ...
                                res = requests.post(url=url, json=jsondata, headers=header, verify=False, timeout=10) # 拼接完字典后发送请求,json形式。data形式的请求：res = requests.post(url, data=json.dumps(todict), headers=header, verify=False)
                        else:                                                                                                         ##############
                            if item["保存id"] == "是":
                                res = requests.post(url=url, data=jsondata, headers=header, verify=False, timeout=10)
                                id = res.json()["demandId"]
                            elif item["沿用id"] == "是":
                                try:
                                    if jsondata["demandId"]:         # 判断传参中参数id是否为demandId，若为是，则demandId=id
                                        jsondata["demandId"] = str(id)
                                    else:
                                        pass
                                except:
                                    if jsondata["id"]:                # 判断传参中参数id是否为id，若为是，则id=id
                                        jsondata["id"] = str(id)
                                    else:
                                        pass
                                res = requests.post(url=url, data=jsondata, headers=header, verify=False, timeout=10)
                    ...
                    try:
                        if item["json"] == "是":
                            res = requests.get(url=url, json=jsondata, headers=header, verify=False, timeout=10)
                        else:
                            res = requests.get(url=url, data=jsondata, headers=header, verify=False, timeout=10)
                    except:
