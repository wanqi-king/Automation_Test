...
class OperateReport:
    ...
    def detail(self, worksheet, info):
        ...
        worksheet.set_column("D:D", 50)#请求体列
        worksheet.set_column("E:E", 30)#请求地址列
        ...
        for item in info:
            if item["结果"] == "失败":
                ...
                _write_center_Yellow(worksheet, "A" + str(temp), item["结果"], self.wd)#如果用例异常跳过，标记为黄色
            else:
                _write_center_Green(worksheet, "A" + str(temp), item["结果"], self.wd)#用例正确，为绿色
...
def pass_format(wd):#成功用例绿色高亮显示,绿底黑字
    return wd.add_format({'bg_color': 'lime', 'align': 'center', 'font_color': 'black', 'bold': 1, 'underline': 1, 'font_size': 12})

def skip_format(wd):#异常用例黄色高亮显示,黄底黑字
    return wd.add_format({'bg_color': 'yellow', 'align': 'center', 'font_color': 'black', 'bold': 1, 'underline': 1, 'font_size': 12})
...
def _write_center_Green(worksheet, cl, data, wd):#_write_center(工作簿, 单元格位置, 数据, 样式)
    return worksheet.write(cl, data, pass_format(wd))

def _write_center_Yellow(worksheet, cl, data, wd):
    return worksheet.write(cl, data, skip_format(wd))
